 ...
 * SKIP LOCKED + 배치 사이즈 설정 최적화 후 업데이트.
 */
...
class OrderOutboxPublisherTest {
        ...
        void setUp() {
                ...
                ReflectionTestUtils.setField(orderOutboxPublisher, "batchSize", batchSize);
        }
        ...
        class PublishSuccessTest {
                ...
                void publish_ShouldSend_ToKafka_AndMark_Published() throws Exception {
                        ...
                        given(outboxRepository.findBatchForPublish(batchSize))
                                        .willReturn(List.of(outbox));

                        CompletableFuture<SendResult<String, EventEnvelope>> future = CompletableFuture
                        ...
                        then(kafkaTemplate).should(times(1)).send(
                                        ...
                                        org.mockito.ArgumentMatchers.argThat((EventEnvelope envelope) -> {
                                                ...
                                                assertThat(envelope.getEventType())
                                                                .isEqualTo(EventType.valueOf(eventType));
        ...
        class PublishFailureTest {
                ...
                void publish_WhenPartialFailure_ShouldMark_Published_AndFailed_Separately() throws Exception {
                        ...
                        given(kafkaTemplate.send(eq(topic), eq(outbox1.getAggregateId().toString()),
                                        any(EventEnvelope.class)))
                                        .willReturn(successFuture);
                        given(kafkaTemplate.send(eq(topic), eq(outbox2.getAggregateId().toString()),
                                        any(EventEnvelope.class)))
                                        .willReturn(failureFuture);
                        given(kafkaTemplate.send(eq(topic), eq(outbox3.getAggregateId().toString()),
                                        any(EventEnvelope.class)))
                                        .willReturn(successFuture);
        ...
        class BatchProcessingTest {
                ...
                @Test
                @DisplayName("설정된 배치 사이즈로 이벤트를 조회해야 한다")
                void publish_ShouldProcess_WithConfiguredBatchSize() {
                        // given
                        given(outboxRepository.findBatchForPublish(batchSize))
                                        .willReturn(List.of());
                        ...
                        // then
                        then(outboxRepository).should(times(1)).findBatchForPublish(batchSize);
                }

                @Test
                @DisplayName("SKIP LOCKED를 사용하는 findBatchForPublish가 호출되어야 한다")
                void publish_ShouldUse_SkipLockedQuery() {
                        // given
                        given(outboxRepository.findBatchForPublish(anyInt()))
                                        .willReturn(List.of());
                        ...
                        // then
                        then(outboxRepository).should(times(1)).findBatchForPublish(anyInt());
                }
        }
        ...
        private OrderOutbox createOutbox(UUID eventId, String eventType, String aggregateType, UUID aggregateId,
                        String payload, String producer, String traceId, String version, Instant createdAt,
                        OutboxStatus status) {
